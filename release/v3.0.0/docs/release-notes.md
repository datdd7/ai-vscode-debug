...
**541/541 unit tests. 72/73 E2E tests. 38 operations. 0 moderate+ vulnerabilities.**

#### What's New vs v3.0.0-alpha.1

- **Code quality audit**: Replaced all `console.log` with structured `logger` utility
- **frameDown() fix**: Throws `DebugError(OPERATION_FAILED)` at frame 0 instead of silent no-op
- **CI security gate**: Blocks on moderate+ vulnerabilities (`npm audit --omit=dev`)
- **Unit tests**: 541/541 (100% coverage threshold enforced, up from 207)
- **E2E tests**: 72/73 PASS (A3 restart skip tracked as issue #42)
