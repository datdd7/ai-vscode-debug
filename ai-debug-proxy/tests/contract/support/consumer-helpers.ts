 ...
 * TODO(temporary scaffolding): Replace with '@seontechnologies/pactjs-utils' exports
 * when the package is published and installed.
 */
...
export const createProviderState = ({ name, params }: ProviderStateInput): [string, JsonMap] =>
    [name, toJsonMap(params)];

export const setJsonContent =
    ({ body, headers, query }: JsonContentInput) =>
    (builder: JsonContentBuilder): void => {
        if (query && builder.query) builder.query(query);
        if (headers) builder.headers(headers);
        if (body !== undefined) builder.jsonBody(body);
    };
